...
const PlaceOrder = () => {
  ...
  const { getTotalCartAmount, token, book_list, cartItems, url, setCartItems } = useContext(StoreContext);
  ...
  const placeOrder = async (e) => {
    ...
    try {
      ...
      const paymentResp = await axios.post("http://localhost:4000/api/payment/create", {
        amount: getTotalCartAmount() === 0 ? 0 : getTotalCartAmount() + 5
      });

      const order = paymentResp.data.order;
